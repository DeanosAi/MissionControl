-- Mission Control VPS v2 — Database Migration
-- Milestones I through M
-- Run on VPS Postgres before deploying v2.
...
-- Milestone J: Ideas with Research
CREATE TABLE IF NOT EXISTS mission_control.ideas (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  title TEXT NOT NULL,
  description TEXT,
  status TEXT NOT NULL DEFAULT 'submitted' CHECK (status IN ('submitted', 'researching', 'researched', 'building', 'built', 'archived')),
  research_data JSONB,
...
CREATE INDEX IF NOT EXISTS idx_automations_next_run ON mission_control.automations(next_run) WHERE status = 'active';

CREATE TABLE IF NOT EXISTS mission_control.automation_runs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  automation_id UUID NOT NULL REFERENCES mission_control.automations(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_automation_runs_automation_id ON mission_control.automation_runs(automation_id);
...
CREATE INDEX IF NOT EXISTS idx_n8n_runs_started ON mission_control.n8n_workflow_runs(started_at DESC);
